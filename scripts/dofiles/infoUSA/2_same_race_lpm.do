...
reghdfe  same_race  Minority_choice  White_choice  , absorb(i.gender i.education_level i.inquiry_order Address)     cl(CBSA_downtown) level(90) 
...
* 2 above median
    ...
    forvalues k =1/2{
        ...
        matrix define `vars'_`k'=J(5,3,.)
        matrix colnames `vars'_`k'= coef lci uci 
        matrix rownames  `vars'_`k'= choice min afam hisp white  

        reghdfe  same_race  choice if  median_`vars'==`k', absorb(i.gender i.education_level i.inquiry_order Address)     cl(CBSA_downtown) level(90) 

        nlcom (RR_choice   : (_b[_cons]  )     / (_b[_cons] + _b[choice] )  ) , post
...
foreach vars in Downtown Suburb Midwest Northeast South West {
        ...
        matrix define `vars'=J(5,3,.)
        matrix colnames `vars'= coef lci uci 
        matrix rownames  `vars'= choice min afam hisp white  

        reghdfe  same_race  choice  if  `vars'==1, absorb(i.gender i.education_level i.inquiry_order Address)     cl(CBSA_downtown) level(90) 
        ...
        reghdfe  same_race  Minority_choice Minority White_choice if  `vars'==1, absorb(i.gender i.education_level i.inquiry_order Address)     cl(CBSA_downtown) level(90) 
        ...
        reghdfe  same_race  AfAm_choice Hispanic_choice AfAm Hispanic White_choice  if  `vars'==1, absorb(i.gender i.education_level i.inquiry_order Address)     cl(CBSA_downtown) level(90) 
