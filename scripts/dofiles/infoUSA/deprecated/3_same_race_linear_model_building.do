...
rename _merge _merge_building

rename race_renter race_renter_building

merge n:1 StreetAddress Zip_Code using "stores/infoUSA_wru.dta"

rename _merge _merge_unit

save "stores/matchedinquiries_opportunities_tract_infoUSA_wru_building.dta",replace



replace race_renter=race_renter_building if race_renter==""
...
matrix define RR_rel_full_sample=J(4,3,.)
matrix colnames  RR_rel_full_sample= coef lci uci  
matrix rownames  RR_rel_full_sample= min afam hisp white  
...
save "stores/matrices/temp/RR_rel_full_sample_building.dta"  , replace
...
* 2 above median
     ...
     *Save to a matrix
        ...
        save "stores/matrices/temp/`vars'_`k'_building.dta"  , replace
...
foreach vars in Downtown Suburb Midwest Northeast South West {
        ...
        matrix define `vars'=J(4,3,.)
        matrix colnames `vars'= coef lci uci 
        matrix rownames  `vars'= min afam hisp white  
        ...
        *Save to a matrix
        ...
        save "stores/matrices/temp/`vars'_building.dta"  , replace
