...
gen qwhitepopchange=(whitepopchange<=-0.10)
replace qwhitepopchange=2 if whitepopchange>-0.10 & whitepopchange<0.10
replace qwhitepopchange=3 if whitepopchange>=0.10
...
*Tables of coeficients
  ...
  #delimit ; 
esttab modelRR
         using "${path_tables}/table_whitepopchange_10.tex", 
         style(tex) 
         ...
         stats(mean_white1
               ...
               labels("Mean Response (White) $\Delta White Pop< -0.10$"
                      "Mean Response (White) $\Delta White Pop\in [-0.10,0.10]$"
                      "Mean Response (White) $\Delta White Pop>0.10$"
                     "\hline Gender" 
                     "Education Level" 
                     "Inquiry Order"
                     "Address"
         ...
         eqlabels(,none)
         varlabels(
                   RR_AfAm_qwhitepopchange1  "African American $\times \Delta White Pop< -0.10$"
                   RR_AfAm_qwhitepopchange2  "African American $\times \Delta White Pop\in [-0.10,0.10]$"
                   RR_AfAm_qwhitepopchange3  "African American $\times \Delta White Pop>0.10$"
                   RR_Hispanic_qwhitepopchange1  "Hispanic/LatinX $\times \Delta White Pop< -0.10$"
                   RR_Hispanic_qwhitepopchange2  "Hispanic/LatinX $\times \Delta White Pop\in [-0.10,0.10]$"
                   RR_Hispanic_qwhitepopchange3  "Hispanic/LatinX $\times \Delta White Pop> 0.10$"
                   ) 
         starl(* 0.1 ** 0.05 *** 0.01)   
