...
egen quartile_rent_Cbsa=xtile(rent), n(`quartiles') 
...
nlcom 	  (RR_AfAm_rent1: _b[AfAm_rent1]/`mean1') ///
	...
	* Results into a Matrix  
	...
	graph export views/quartiles_across_CBSA_rent_black.png, width(800) replace
...
* Results into a Matrix  
	...
	twoway  (rarea hisp_lci hisp_uci quartiles, color(gray%20))  ///
			...
			scheme(plotplain) ///
			yscale(range(-0.20(.025)0.025)) ylabel(-0.20(0.025)0.025) 
	graph export views/quartiles_across_CBSA_rent_hisp.png, width(800) replace
