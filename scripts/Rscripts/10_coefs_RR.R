...
# Odds Ratios -------------------------------------------------------------
dta_rr<-data.frame(dta) %>% dplyr::select(cbsa_st,
                                          cbsas_afam_lci,
                                          cbsas_afam_coef,
                                          cbsas_afam_uci,
                                          cbsas_hispanic_lci,
...
dta_rr<- dta_rr %>% mutate(cbsa_st=fct_reorder(cbsa_st,-cbsas_afam_coef))
aa<-ggplot(dta_rr, aes(x = cbsas_afam_coef, y = cbsa_st, xmin = cbsas_afam_lci, xmax = cbsas_afam_uci)) +
    geom_point(size=2) +
    ...
    coord_cartesian(xlim = c(-38,24), clip = 'off') +
...
aa    
dta_rr<- dta_rr %>% mutate(cbsa_st=fct_reorder(cbsa_st,-cbsas_hispanic_coef))
hisp<-ggplot(dta_rr, aes(x = cbsas_hispanic_coef, y = cbsa_st, xmin = cbsas_hispanic_lci, xmax = cbsas_hispanic_uci)) +
      geom_point(size=2) +
      geom_errorbarh(height=.2) +
      #scale_colour_manual(values=c("#3B4992FF", "#EE0000FF", "#631879FF", "#CCCCCC")) +
      #scale_shape_manual(values=c(15,16,17,18)) +
...
ggsave("views/RR_cbsas.pdf",cmb,height = 16,width = 11)
